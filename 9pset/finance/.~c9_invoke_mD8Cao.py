...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO transactions (date, user)")
...
def register():
    ...
    if request.method == "POST":
        ...
        rows = db.execute("SELECT count(*) FROM users WHERE username = ?", request.form.get("username"))
...
def sell():
    ...
    if request.method == "POST":
        ...
        if not request.form.get("symbol") and not request.form.get("shares"):
            return apology("must provide symbol and/or shares", 403)
        ...
        shares_symbol = db.execute("SELECT symbol FROM shares_purchase WHERE user_id=? AND symbol=?", session["user_id"], request.form.get("symbol"))
        ...
        shares_info = db.execute("SELECT * FROM shares_purchase WHERE user_id=? AND symbol=?", session["user_id"], request.form.get("symbol"))
        ...
        total_p = shares_info[0]["price"] * num
        cash =  user_cash[0]["cash"] + (int(request.form.get("shares")) * shares_info[0]["price"])

        # Update cash, shares and total or delete entire shares
        if num:
            db.execute("UPDATE shares_purchase SET shares_amount=?, total_purchase=? WHERE symbol=? AND user_id=?", num, total_p, request.form.get("symbol"), session["user_id"])
            db.execute("UPDATE users SET cash=? WHERE id=?", cash, session["user_id"])
        else:
